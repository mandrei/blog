...
class JournalUpdate extends \Controller {
    ...
    public function process_update($journal_id)
    {
        ...
        $inputs = array(
                        ...
                        'journal_header_image'  => Input::file('journal_header_image'),
        ...
        */
        $validator = new \CoreApp\Website\Validations\Journal(array(
                                                                    ...
                                                                    'journal_header_image' => $inputs['journal_header_image']
        ...
        try{
            ...
            DB::transaction(function() use($website_journal_model,$journal_post_category_model,$journal_id,$journal_post_tag_model,$inputs)
            {
                ...
                //If  Journal header image isset
                if(isset($inputs['journal_header_image'])) $journal_header_image_extension = $inputs['journal_header_image']->getClientOriginalExtension();
                ...
                //Get Journal header image extension for update from input if new picture is selected else same extension remains
                if( Input::file('journal_header_image') )
                {
                    $journal_head_image_extension = $inputs['journal_header_image']->getClientOriginalExtension();

                }//if new picture is selected
                else
                {

                    $journal_head_image_extension = $journal_post->journal_header_image_extension;

                }//else same picture extension remains


                //Update journal
                $website_journal_model->update_journal($journal_id,$inputs['meta_description'],$inputs['meta_keywords'],$inputs['post_url'],$inputs['title'],$inputs['post'],$journal_image_extension,$journal_head_image_extension,$inputs['featured']);
                ...
                if( Input::file('main_journal_image') )
                {
                    ...
                    if(is_dir('cms/journal/'.$journal_id))
                    {
                         ...
                         if(is_file('cms/journal/'.$journal_id.'/journal_photo_small/'.$journal_post->journal_image_extension)) {
                            unlink('cms/journal/'.$journal_id.'/journal_photo_small/'.$journal_post->journal_image_extension);
                        }
                        ...
                        Image::make('cms/journal/'.$journal_id.'/'.$main_journal_image_extension)->resize(null, 70,true)->save('cms/journal/'.$journal_id.'/journal_photo_small.'.$main_journal_image_extension);
                ...
                //If new journal header image is added
                if( Input::file('journal_header_image') )
                {
                    /*
                     * Delete image
                     */
                    if(is_dir('cms/journal_header/'.$journal_id))
                    {


                         if(is_file('cms/journal_header/'.$journal_id.'/journal_header/'.$journal_post->journal_header_image_extension)) {
                            unlink('cms/journal_header/'.$journal_id.'/journal_header/'.$journal_post->journal_header_image_extension);
                        }

                        
                    }//if has journal header picture


                     //Add header journal image
                    Input::file('journal_header_image')->move('cms/journal_header/'.$journal_id,'/'.$journal_header_image_extension);

                    Image::make('cms/journal_header/'.$journal_id.'/'.$journal_header_image_extension)->resize(1920, 650,false)->save('cms/journal_header/'.$journal_id.'/journal_header.'.$journal_header_image_extension);

                    unlink('cms/journal_header/'.$journal_id.'/'.$journal_header_image_extension);

                }//if new journal header image was added
